...
export default function FeaturedCollection({module}: Props) {
  ...
  const {moduleDescription, moduleHeading, collection, linkText} = module;
  ...
  return (
    <div className="featured-collection page-width flex flex-col gap-[34px] md:flex-row md:overflow-hidden">
      <div className="flex w-full flex-col items-center justify-center gap-4 md:w-1/3 md:items-start lg:w-1/4">
        {moduleHeading && (
        ...
        {collection.slug && (
          <span>
            ...
            >
              {linkText ? linkText : collection.title}
            </Link>
          </span>
        )}
      </div>
      <div className="-mx-5 flex w-full flex-col gap-[26px] md:w-2/3 md:overflow-hidden lg:w-3/4">
        <div>
