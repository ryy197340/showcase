...
function ProductInlineLinkContent({
  ...
  if (!selectedVariant.availableForSale && linkAction !== 'link') {
    return (
      <>
        ...
        <span className="color-white rounded-xs ml-[0.25em] bg-lightGray px-1 py-0.5 text-xs font-bold text-red">
          Sold out
        </span>
      </>
    );
  ...
  const LinkContent = useMemo(
    () => (
      <span
        className={clsx(
          'rounded-xs inline-flex place-content-center items-center p-0.5 leading-none duration-200 ease-out',
          'hover:opacity-80',
        )}
    ...
    <Tippy
      ...
      render={() => {
        if (linkAction === 'addToCart') {
          return <Tooltip label={`Add to bag: ${title}`} tone="dark" />;
        }
