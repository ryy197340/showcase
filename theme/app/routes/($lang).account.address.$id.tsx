...
export const action: ActionFunction = async ({request, context, params}) => {
  ...
  if (address.firstName) {
    const validation = validateProfileName(address.firstName, 'First name');
    if (!validation.isValid) {
      return badRequest<ActionData>({formError: validation.error});
    }
  }

  if (address.lastName) {
    const validation = validateProfileName(address.lastName, 'Last name');
    if (!validation.isValid) {
      return badRequest<ActionData>({formError: validation.error});
    }
  }
  ...
  if (address.phone) {
    const validation = validatePhone(address.phone);
    if (!validation.isValid) {
      return badRequest<ActionData>({formError: validation.error});
    }
  }

  // Validate address-specific fields
  if (address.address1) {
    const validation = validateAddressField(address.address1, 'Street address');
    if (!validation.isValid) {
      return badRequest<ActionData>({formError: validation.error});
    }
  }
  ...
  if (address.address2) {
    const validation = validateAddressField(address.address2, 'Address line 2');
    if (!validation.isValid) {
      return badRequest<ActionData>({formError: validation.error});
    }
  }
  ...
  if (address.city) {
    const validation = validateAddressField(address.city, 'City', 50);
    if (!validation.isValid) {
      return badRequest<ActionData>({formError: validation.error});
    }
  }
  ...
  if (address.zip) {
    const validation = validatePostalCode(address.zip);
    if (!validation.isValid) {
      return badRequest<ActionData>({formError: validation.error});
    }
  }
  ...
  if (address.company) {
    const validation = validateAddressField(address.company, 'Company', 80);
    if (!validation.isValid) {
      return badRequest<ActionData>({formError: validation.error});
    }
  }
      ...
      <div className="mt-10 flex justify-end">
        <div className="flex gap-2">
          <Button
            to=".."
            mode="outline"
            className={clsx([squareButtonStyles({tone: 'default'}), ''])}
            type="button"
            preventScrollReset
          >
            Cancel
