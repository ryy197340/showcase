...
export const SearchCustomerScreen = () => {
  ...
  const handleCustomer = async (customer) => {
    const customerUuid = customer.customerUuid;

    if (!customerUuid || !params?.authToken) {
      api.toast.show("Invalid parameters. Please try again.");
      return;
    }

    // api.toast.show(`customer... ${JSON.stringify(customer)}`);
                  ...
                  {searchResults
                    ...
                    .map((customer) => (
                      <Stack
                        ...
                        <Button
                          title={`Select Customer ${customer.firstName}`}
                          onPress={() => handleCustomer(customer)}
                          loading={isAdding}
                          disabled={isAdding}
                        />
                      </Stack>
